   ...
   function showUploadImage(uploadResultArr) {
      ...
      for (let i = 0; i < uploadResultArr.length; i++) {
         ...
         let fileCallPath = obj.uploadPath.replace(/\\/g, '/') + "/s_"
               + obj.uuid + "_" + obj.orgImg;
         str += "<div style='margin: 5px;'>"
         str += "<img src='${contextPath}/product/display?fileName="
               + fileCallPath + "' width='150px' height='150px' >";
         str += "</div>";
               ...
               success : function(list) {
                  ...
                  if (num == 1) {
                     for (let i = 0; i < list.length; i++) {
                        str += '<div class="cate2List" onclick="openCate2(event,'
                              + list[i].cateCode
                              + ", '"
                              + list[i].cateName
                              + "'"
                     ...
                     for (let i = 0; i < list.length; i++) {
                        str += '<div class="cate3List" onclick="openCate3(event,'
                              + list[i].cateCode
                              + ", '"
                              + list[i].cateName
                              + "'"
...
<body>
   ...
   <form id="frm" action="${contextPath }/product/prodRegister"
      ...
      <section class="eeRGVw">
         ...
         <div class="flex">
            <div class="size_150">
            카테고리<span class="redmen">*</span> <input type="text" id="cateCode"
                  name="cateCode" style="display: none;">
            </div>

            <div class="catewrap" style="margin-bottom: 20px;">

               <div class="cateBox1">
                  <c:forEach var="dto" items="${cateList }">
                     <div class="cate1List"
                        onclick="openCate(event,'${dto.cateCode}','${dto.cateName }')">${dto.cateName }</div>
                  </c:forEach>
               </div>

               <div class="cateBox2"></div>

               <div class="cateBox3"></div>
         ...
         <div>
            <label>선택한 카테고리 : </label> <span
               id="userSel1"></span> <span id="userSel2"></span> <span
               id="userSel3"></span>
         </div>
